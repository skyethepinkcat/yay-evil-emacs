...
~:ensure nil~ when configuring the built-in packages.  #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package org-roam :config
    ...
    (cl-defmethod org-roam-node-my-tags ((node org-roam-node))
      (mapcar (lambda (arg) (concat "#" arg)) (remove "ATTACH" (org-roam-node-tags node)))
      )
    (setq org-roam-node-display-template
          (concat "${title:90} "
                  (propertize "${my-tags:10}" 'face 'org-tag)))
...
#+BEGIN_SRC emacs-lisp
    (require 'org-roam
    (define-keymap org-roam-mode-map
                      :prefix "C-c r"
